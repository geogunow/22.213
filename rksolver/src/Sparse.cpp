...
/*
   Operator overloading for matrix-matrix addition A(self) + B = C
   */
Sparse Sparse::operator + (Sparse B) const
{   
    // create new sparse object
    Sparse C = Sparse(_M, _N);

    // check dimensions
    if(B.size() != _M)
    {
        std::cout << "Error: Matrix dimension mismatch" << endl;
        return C;
    }
    ...
    for(int i=0; i < _M; i++)   
    {
        ...
        for(it = B.rowStart(i); it != B.rowEnd(i); ++it)
        {
            // load matrix elements
            int j = it->first;
            double v = it->second;
...
/*
   Operator overloading for matrix-matrix subtraction A(self) - B = C
   */
Sparse Sparse::operator - (Sparse B) const
{   
    // create new sparse object
...
/*
   Function to deep copy this matrix
   */
Sparse Sparse::copy()
{   
    // create new sparse object
    Sparse New = Sparse(_M, _N);
...
/* 
   Function that returns an iterator to the starting column of a row
*/
std::map<int,double>::const_iterator Sparse::rowStart(int i)
{
    return _vals[i].begin();
}

/* 
   Function that returns an iterator to the starting column of a row
*/
std::map<int,double>::const_iterator Sparse::rowEnd(int i)
{
    return _vals[i].end();
}
