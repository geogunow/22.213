...
'''
function to view nonzero elements of matrix A with N cells, suspending program
execution to display the plot
Arguments:
    A:  Matrix to view
...
def view_matrix(A, N):
    ...
    plt.spy(Asparse, marker='.', markersize = 30.0/sqrt(N))
    plt.show()
...
'''
def calculate_l2_errors(cases, reference, N):

    l2_errors = np.zeros( len(cases) )
...
'''
#NOTE: This assumes groups on the outside
def calculate_nodal_avg(values, refinement):
    
    L = len(values)/refinement
    avg_vals = np.zeros(L)
